...
public class GameManager : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        EvolutionSceneManager.I.OnStartEvolution += ()=> SetState(GameState.Evolution);
        EvolutionSceneManager.I.OnEndEvolution += ()
            =>
        {

            //menuController.PartyMenu1.SetPartyData()
            SetState(GameState.freeRoam);
        };
    ...
    public void EndOfBattle()
    {
        ...
        StartCoroutine(FadeOutFromBattle());


    }


    public IEnumerator FadeOutFromBattle()
    {
        battleSystem.SetActive(false);
        //eventSystem.gameObject.SetActive(true);
        yield return null;
        SetState(GameState.freeRoam);
        StartCoroutine( fader.FadeOut(.2f));
        var party = player.GetComponent<PokemonParty>();
        yield return party.CheckForEvolutions();

    }

    public IEnumerator FadeInToFreeRoam()
    {
